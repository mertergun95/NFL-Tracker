...
import type { Bankroll, Bet, Settings, Transaction } from '../core/types';

/**
 * Single source of truth for everything loaded from the device store.
 *
 ...
 * instantly without touching storage.
 *
 * This is the phone's copy of the web module's provider. It differs in exactly
 * one way: the theme is not applied here. On the web the module writes
 * `<html data-theme>`; on the phone the app-wide `ThemeProvider` owns that, so
 * the bet tracker's theme setting is forwarded to it by the settings screen
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const saveBankroll = useCallback(async (input: Partial<Bankroll> & { name: string }) => {
